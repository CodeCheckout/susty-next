...
const categorySecond = ["Clothes", "Shoes", "Bags", "Accessories", "Beauty", "Grooming", "Toys & games", "Baby care", "Strollers", "Chairs", "School supplies", "Textile", "Home accessories", "Video games & consoles", "Games & puzzles", "Music & video", "Books"]
const categoryThird = ["Coats & jackets", "Jumpers & sweaters", "Suits & blazers", "Dresses", "Skirts", "Jeans", "Pants & leggings", "Shorts & cropped pants", "Swimwear", "Boots", "Heels", "Flats", "Ankle boots", "Flip-flops", "Handbags", "Backpacks", "Tote bags", "Clutches", "Purses & wallets", "Makeup bags", "Jewelry", "Watches", "Belts", "Sunglasses", "Gloves", "Hats & caps", "Makeup", "Perfume", "Face care", "Pants", "Shorts", "T-shits", "Sneakers", "Sport shoes"]
...
const AddProduct = () => {
    ...
    const onSubmitClick = () => {
        ...
        imageList.map((singleImage) => {
            ...
            const fileRef = ref(storageRef, `images/${Date.now()}-${singleImage.name}`);

            uploadBytes(fileRef, singleImage.file).then((snapshot) => {
    ...
    useEffect(() => {
        if (imageList.length > 0
            &&
            uploadedFileCount === imageList.length) {
            addProductHandler()
        }
    ...
    const addProductHandler = async () => {
        if (title.trim() === "" || description.trim() === "" || categoryOne.trim() === "" || categoryTwo.trim() === "" || categoryThree.trim() === "" || brand.trim() === "" || size.trim() === "" || condition.name.trim() === "" || price.trim() === "" || currency.trim() === "" || imageList.length === 0) {
    ...
    return (<div className={`w-screen grid place-content-center font-susty p-4 w-[98vw]`}>
        <>
            ...
            <div className='max-w-4xl pb-4'>
                <TitleAndDescription title={title} setTitle={setTitle} description={description}
                                     setDescription={setDescription}/>
            </div>
            <div className='max-w-4xl pb-4'>
                <CategoryBrandCondition categoryOne={categoryOne} setCategoryOne={setCategoryOne}
                                        categoryTwo={categoryTwo} setCategoryTwo={setCategoryTwo}
                                        categoryThree={categoryThree} setCategoryThree={setCategoryThree}
                                        selected={condition} setSelected={setCondition} brand={brand}
                                        setBrand={setBrand} size={size} setSize={setSize}/>
            </div>
            <div className='max-w-4xl pb-4'>
                <PriceAndSwapping price={price} setPrice={setPrice} setCurrency={setCurrency}
                                  setSwapping={setSwapping} currency={currency} swapping={swapping}/>
            </div>
