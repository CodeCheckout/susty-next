...
const itemsStaticData = [{
    ...
    profileImg: "https://c1.wallpaperflare.com/preview/568/71/709/face-facial-hair-fine-looking-guy.jpg",
...
const ProductItemsFilterSection = () => {
    ...
    useEffect(() => {
        ...
        if (sectionCount === 1) {
            ...
            const title = mainsectionpage.split('-').join(' ').trim().toLowerCase()
            setSectionName(title[0].toUpperCase() + title.substring(1, title.length))

            setPagePathSet([{id: "32bdfd12-d3d5-40ab-9440-e160bb4e413b", name: [mainsectionpage], href: "#"}])
        }
        if (sectionCount === 2) {
            ...
            setPagePathSet([{
                ...
                href: "#"
            }, {id: "542118b3-5107-44e6-9ab8-a6fd4e121ba1", name: [sectionpage], href: "#"}])
        }
        if (sectionCount === 3) {
            const {mainsectionpage, sectionpage, subsectionpage} = router.query
            const title = subsectionpage.split('-').join(' ').trim().toLowerCase()
            setSectionName(title[0].toUpperCase() + title.substring(1, title.length))
            ...
            setPagePathSet([{
                ...
                name: [mainsectionpage],
                href: "#"
            }, {id: "2d2e6c89-e944-4e61-bc6d-22e507cbc1ea", name: [sectionpage], href: "#"}, {
    ...
    return (
        <>
            <ProductCategoryFilterWrapper topic={sectionName} noOfItems={items.length} pagePath={pagePathSet}
                                          productResult={productResult} setProductResult={setProductResult}>
                <div className={'px-3 sm:px-5 lg:px-2'}>
                    <div
                        className={"grid grid-cols-2 sm:grid-cols-3 md:grid-cols-3 gap-3 sm:gap-2 lg:gap-1 justify-items-center py-5"}>
                        {productResult.map((item) => (
                            <div key={item._id}>
                                <CommonItemCard userId={item.owner} profileAlt={"user"} src={item.images} alt={item.alt}
                                                price={item.price} size={item.size}
                                                brand={item.brand} item={item}/>
                            </div>
                        ))}
                    </div>
