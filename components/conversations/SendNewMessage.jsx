...
const peopleStaticData = [{id: 1, name: 'Kade Cooper'}, {id: 2, name: 'Arlene Mccoy'}, {id: 3, name: 'Devon Webb'}, {
    id: 4,
    name: 'Tom Cook'
}, {id: 5, name: 'Tanya Fox'}, {id: 6, name: 'Hellen Schmidt'}, {id: 7, name: 'Sebastian Carlos'}, {
    id: 8,
    name: 'Kris Raven'
}, {id: 9, name: 'Amir Diafi'}, {id: 10, name: 'David Rodenas'},]
...
const SendNewMessage = () => {
    ...
    const filteredPeople = query === '' ? people : people.filter((person) => person.name.toLowerCase()
        .replace(/\s+/g, '')
    ...
    return (
        <div className=' p-4 px-4 md:max-w-4xl m-auto pt-8'>
            ...
            <div className={"bg-white shadow-md"}>
                                ...
                                <div className="relative mt-1">
                                    <div
                                        ...
                                        <Combobox.Button
                                            ...
                                            <HiChevronUp
                                                className={`h-5 w-5 text-gray-400 ${opened ? '' : 'rotate-180'}`}
                                                aria-hidden="true"
                                            />
                                    ...
                                    >
                                        <Combobox.Options
                                            ...
                                            {filteredPeople.length === 0 && query !== '' ? (<div
                                                className="relative cursor-default select-none py-2 px-4 text-gray-700">
                                                Nothing found.
                                            </div>) : (filteredPeople.map((person) => (
                                                <Combobox.Option
                                                    key={person.id}
                                                    className={({active}) => `relative cursor-default select-none py-2 pl-10 pr-4 text-sm 
                                                ...
                                                >
                                                    {({selected, active}) => (<>
                                                        ...
                                                        {selected ? (<span
                                                            className={`absolute inset-y-0 left-0 flex items-center text-sm pl-3 ${active ? 'text-white' : 'text-red-600'}`}
                                                        >
                ...
                <div
                    className={"grid grid-cols-6 lg:grid-cols-10 gap-2 items-center border border-t-4 border-gray-100 pr-4 relative"}>
                    <FiCamera
                    ...
                    <input
                        ...
                        className={"col-start-2 col-end-7 lg:col-start-2 lg:col-end-11 px-5 py-3 my-5 text-sm bg-gray-100 rounded-lg focus:outline-none"}
                        placeholder={"Write a message here"}/>
                    <HiArrowRight
                        className={`col-start-6 lg:col-start-10 w-5 h-5 font-semibold absolute ${msgInput.length > 0 ? 'text-susty' : 'text-gray-300'}`}/>
                </div>
            </div>
        </div>
    );
