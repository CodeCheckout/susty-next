...
const NewsFeedItems = () => {
    ...
    return (
        <div>
            ...
            <div
                className={"grid grid-cols-2 grid-rows-5 sm:grid-cols-3 sm:grid-rows-3 md:grid-cols-3 md:grid-rows-3 lg:grid-cols-5 lg:grid-rows-2 gap-3 justify-items-center py-5"}>
                {items.map((item) => (
                    <div key={item.id}>
                        <NewsFeedItemCard username={item.username} profileImg={item.profileImg}
                                          profileAlt={item.profileAlt} src={item.src} alt={item.alt} price={item.price}
                                          size={item.size}
                                          brand={item.brand}/>
                    </div>
                ))}
